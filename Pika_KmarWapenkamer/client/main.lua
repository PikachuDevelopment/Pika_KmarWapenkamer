...
Citizen.CreateThread(function()
	...
	while true do
		...
		if not Config.OnlyPolicemen or (Config.OnlyPolicemen and ESX.PlayerData["job"] and ESX.PlayerData["job"]["name"] == "kmar") then

			local ped = PlayerPedId()
			local pedCoords = GetEntityCoords(ped)

			local dstCheck = GetDistanceBetweenCoords(pedCoords, Config.wapens["x"], Config.wapens["y"], Config.wapens["z"], true)

			if dstCheck <= 5.0 then
				sleepThread = 5

				local text = "Wapenkamer"

				if dstCheck <= 0.5 then
					text = "[~g~E~s~] Wapenkamer"
				...
				ESX.Game.Utils.DrawText3D(Config.wapens, text, 0.6)
			end
		end
...
Openkmarwapens = function()
	PlaySoundFrontend(-1, 'BACK', 'HUD_AMMO_SHOP_SOUNDSET', false)

	local elements = {
		{ ["label"] = "Wapens", ["action"] = "weapon_storage" }
	}

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "kmar_menu",
		{
			title    = "Kmar Wapenkamer",
			align    = "center",
			elements = elements
		},
	function(data, menu)
...
OpenWeaponStorage = function()
	...
	local Location = Config.wapens
	local PedLocation = Config.wapenped

	for i = 1, #Config.wapenkamerwapens, 1 do
		local weapon = Config.wapenkamerwapens[i]
	...
	function(data, menu)
		...
		if (DoesEntityExist(closestPed) and closestPedDst >= 5.0) or IsPedAPlayer(closestPed) then
			...
			ESX.ShowNotification("Probeer het opnieuw.")
		...
		if IsEntityPlayingAnim(closestPed, "mp_cop_armoury", "pistol_on_counter_cop", 3) or IsEntityPlayingAnim(closestPed, "mp_cop_armoury", "rifle_on_counter_cop", 3) then
			ESX.ShowNotification("Wacht even aub.")
			return
		end
		...
		if DoesEntityExist(closestPed) and closestPedDst <= 5.0 then
			...
			GiveWeaponToPed(PlayerPedId(), GetHashKey(weaponHash), Config.krijgkogels, false, true)
			SetCurrentPedWeapon(PlayerPedId(), GetHashKey(weaponHash), true)

			ClearPedTasks(closestPed)

			TriggerServerEvent("pika_kmarwapens:giveWeapon", weaponHash)
		end
...
RefreshPed = function(spawn)
	local Location = Config.wapenped

	ESX.TriggerServerCallback("pika_kmarwapens:pedExists", function(Exists)
